...
import { SQUIDGAME_THUMBS } from "@/lib/squidgameThumbs";
...
const SQUIDGAME_DEFAULT = "squidgame-masks";
...
const isSquidGameId = (id: string) => id.startsWith("squidgame-");
...
export default function Home() {
  ...
  // Minecraft, One Piece, Attack on Titan, Game of Thrones and Squid Game are grouped under
  ...
  const gridThemes = themes.filter(
    (t) => !isMinecraftId(t.id) && !isOnePieceId(t.id) && !isAotId(t.id) && !isGotId(t.id) && !isSquidGameId(t.id)
  );
  ...
  const squidgameThemes = themes.filter((t) => isSquidGameId(t.id));
  ...
  const squidgameSelected = isSquidGameId(selectedTheme);
  ...
  const squidgameGroup = squidgameThemes.find((t) => t.id === SQUIDGAME_DEFAULT) ?? squidgameThemes[0];
          ...
          <div className="space-y-10">
            ...
            <div className="pt-8 border-t border-white/[0.06]">
              ...
              <div>
                ...
                {!minecraftSelected && !onepieceSelected && !aotSelected && !gotSelected && !squidgameSelected && (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {squidgameGroup && (
                    <button
                      onClick={() => { if (!squidgameSelected) setSelectedTheme(SQUIDGAME_DEFAULT); }}
                      aria-pressed={squidgameSelected}
                      aria-label="Squid Game themes"
                      className={`px-3.5 py-2.5 rounded-lg border transition-all cursor-pointer ${
                        squidgameSelected
                          ? "border-[#da1b60]/50 ring-1 ring-[#da1b60]/20 bg-[#da1b60]/[0.05]"
                          : "border-white/[0.07] hover:border-white/20"
                      }`}
                      style={{ background: squidgameGroup.background }}
                    >
                      <div className="flex gap-1 justify-center mb-1.5">
                        {squidgameGroup.colors.map((c, i) => (
                          <span key={i} className="w-2 h-2 rounded-full" style={{ background: c }} />
                        ))}
                      </div>
                      <span className={`text-[10px] font-semibold uppercase tracking-wider block text-center ${squidgameSelected ? "text-[#da1b60]" : "text-white/60"}`}>
                        Squid Game
                      </span>
                    </button>
                  )}
                ...
                {/* Squid Game variants */}
                {squidgameSelected && squidgameThemes.length > 0 && (
                  <div className="mt-3 p-3 rounded-xl border border-white/[0.07] bg-white/[0.02]">
                    <p className="text-[11px] font-semibold text-white/35 uppercase tracking-widest mb-3">
                      Squid Game Style
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {squidgameThemes.map((t) => {
                        const variant = t.id.replace("squidgame-", "");
                        const active = selectedTheme === t.id;
                        return (
                          <button
                            key={t.id}
                            ...
                            className={`flex items-center gap-2.5 pl-1.5 pr-3.5 py-1.5 rounded-lg border transition-all cursor-pointer ${
                              active
                                ? "border-[#da1b60]/50 ring-1 ring-[#da1b60]/10 bg-[#da1b60]/[0.04]"
                                : "border-white/[0.07] hover:border-white/20"
                            }`}
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={SQUIDGAME_THUMBS[variant]}
                              alt=""
                              width={28}
                              height={28}
                              className="w-7 h-7 rounded-[3px]"
